    ...
    public class PermutationGA : GA<int>
    {
        ...
        public override void generateAPairOfCrossoveredOffspring(int fartherIdx, int motherIdx, int child1Idx, int child2Idx)
        {
            switch (crossoverType)
            {
                ...
                case PermutationCrossover.Emerget://rate per capacity * distance to choose
                    {
                        List<int> PossibleStation = new List<int>();
                        int[] routingSequence = new int[stations.Length];
                        int[] r2 = new int[stations.Length];
                        ...
                        for (int i = 0; i < stations.Length; i++)
                        {
                            ...
                            for (int j = 0; j < stations.Length; j++)
                            {
                                ...
                                {
                                    d1[i, j] = FromTo[i, j] * 1 / stations[j].RatePerCapacity;
                                    d2[i, j] = FromTo[i, j] * 1/stations[j].RatePerCapacity ;
                                }
                                else
                                {
                                    d1[i, j] = FromTo[i, j]*10000;
                                    d2[i, j] = FromTo[i, j]*10000;
                                }
                ...
                case _2ndCrossOver.TES:
                    {
                        ...
                        for (int a = stations.Length; a < this.numberOfGenes - 2; a++)
                        {
                            if (stations[a - stations.Length].Rate > 0)
                            {
                                ...
                                {
                                    this.chromosomes[child2Idx][a]
                                    = Convert.ToInt32(alpha * this.chromosomes[motherIdx][a]);
                                    this.chromosomes[child1Idx][a]
                                    = Convert.ToInt32((this.chromosomes[fartherIdx][a]) * (1 - alpha) - alpha * TruckCapacity);
                                }
                                else
                                {
                                    this.chromosomes[child1Idx][a]
                                    = Convert.ToInt32(alpha * this.chromosomes[fartherIdx][a]);
                                    this.chromosomes[child2Idx][a]
                                    = Convert.ToInt32((this.chromosomes[motherIdx][a]) * (1 - alpha) - alpha * TruckCapacity);
                                }
                            ...
                            else if (stations[a - stations.Length].Rate < 0)
                            {
                                ...
                                {
                                    this.chromosomes[child2Idx][a]
                                    = Convert.ToInt32((this.chromosomes[motherIdx][a]) * alpha + (1 - alpha) * TruckCapacity);
                                    this.chromosomes[child1Idx][a]
                                    = Convert.ToInt32((this.chromosomes[fartherIdx][a]) * (1 - alpha));
                                }
                                else
                                {
                                    this.chromosomes[child1Idx][a]
                                    = Convert.ToInt32((this.chromosomes[fartherIdx][a]) * alpha + (1 - alpha) * TruckCapacity);
                                    this.chromosomes[child2Idx][a]
                                    = Convert.ToInt32((1 - alpha) * this.chromosomes[motherIdx][a]);
                                }
                            }
                        }
                ...
                case _2ndCrossOver.LVD:
                    {
                        ...
                        for (int a = stations.Length; a < this.numberOfGenes - 2; a++)
                        {
                            if (stations[a - stations.Length].Rate > 0)
                            {
                                ...
                                {
                                    this.chromosomes[child2Idx][a]
                                    = Convert.ToInt32((this.chromosomes[motherIdx][a]) * alpha + (alpha - 1) * TruckCapacity);
                                    this.chromosomes[child1Idx][a] = Convert.ToInt32((1 - alpha) * this.chromosomes[fartherIdx][a]);
                                }
                                else
                                {
                                    this.chromosomes[child1Idx][a]
                                    = Convert.ToInt32((this.chromosomes[fartherIdx][a]) * alpha + (alpha - 1) * TruckCapacity);
                                    this.chromosomes[child2Idx][a] = Convert.ToInt32((1 - alpha) * this.chromosomes[motherIdx][a]);
                                }
                ...
                case _2ndCrossOver.SVD:
                    {
                        ...
                        for (int a = stations.Length; a < this.numberOfGenes - 2; a++)
                        {
                            ...
                            else if (stations[a - stations.Length].Rate < 0)
                            {
                                ...
                                {
                                    this.chromosomes[child1Idx][a]
                                    = Convert.ToInt32((this.chromosomes[motherIdx][a]) * alpha + (1 - alpha) * this.chromosomes[motherIdx][a]);
                                    this.chromosomes[child2Idx][a]
                                    = Convert.ToInt32((1 - alpha) * this.chromosomes[motherIdx][a]);
                                }
                            }
